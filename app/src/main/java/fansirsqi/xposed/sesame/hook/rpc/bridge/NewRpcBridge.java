                                ...
                                if (args != null && args.length == 1 && "sendJSONResponse".equals(innerMethod.getName())) {
                                    try {
                                        ...
                                        if (!(Boolean) XposedHelpers.callMethod(obj, "containsKey", "success")
                                            ...
                                            // 检查是否有errorMsg字段，如果有且为"ok"，则不算错误
                                            if ((Boolean) XposedHelpers.callMethod(obj, "containsKey", "errorMsg")) {
                                                String errorMsg = (String) XposedHelpers.callMethod(obj, "getString", "errorMsg");
                                                if ("ok".equals(errorMsg)) {
                                                    // errorMsg为"ok"时视为成功，不设置错误状态
                                                    return null;
                                                }
                                            }
                                            ...
                                            if ((Boolean) XposedHelpers.callMethod(obj, "containsKey", "error")) {
                                                String errorCode = String.valueOf(XposedHelpers.callMethod(obj, "get", "error"));
                                                if ("1009".equals(errorCode)) {
                                                    Toast.show("紧急！！！请完成滑块验证", false);
                                                    try {
                                                        Thread.sleep(30000);
