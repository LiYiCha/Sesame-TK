...
public class DebugRpc {
    ...
    public DebugRpc(Context context) {
        this.context = context;
        if (context == null) {
            Log.error("DebugRpc: Context is null in constructor");
        }
    }

    public void sendResult(String result) {
        Intent intent = new Intent("com.eg.android.AlipayGphone.sesame.rpcresponse");
        intent.setPackage("fansirsqi.xposed.sesame");
        intent.putExtra("result", result);
        context.sendBroadcast(intent);
        if (context != null) {
            ToastUtil.makeText("Rpc--自定义响应广播成功",1).show();
        }
    }
    //----------------------------------------------------------
    ...
    public void start(String broadcastFun, String broadcastData, String testType) {
        new Thread() {
            ...
            public void run() {
                switch (testType) {
                    case "Rpc":
                        String s = ReserveRpcCall.queryPost(broadcastFun, broadcastData);
                        Log.debug("收到测试消息:\n方法:" + broadcastFun + "\n数据:" + broadcastData + "\n结果:" + s);
                        ...
                        if (context != null) {
                            ...
                            Log.error(TAG,"[Rpc]Context is null, cannot send broadcast");
                        }
                        break;
