...
class AntFarm : ModelTask() {
    ...
    private var npcAnimalTypeMulti: SelectModelField? = null  // NPC多选（智能调度）
    // NPC配置定义
    ...
    // 简单的 NPC 实体类
    private class SimpleNpcEntity(id: String, name: String) : MapperEntity() {
        init {
            this.id = id
            this.name = name
        }
    ...
    override fun getFields(): ModelFields {
        ...
        modelFields.addField(
            SelectModelField(
                ...
                "NPC智能调度(多选,按优先级)",
                linkedSetOf(),
                NpcConfig.getNpcOptionList()
            ).also { npcAnimalTypeMulti = it })
        modelFields.addField(
            BooleanModelField(
                "sendBackAnimal",
                "遣返 | 开启",
    ...
    override suspend fun runSuspend() {
        try {
            ...
            if (npcAnimalType!!.value != NpcConfig.NONE.ordinal
                || npcAnimalTypeMulti?.value?.toList()?.isNotEmpty() == true
            ) {
            ...
            if (kitchen!!.value) {
                ...
                if (AnimalFeedStatus.SLEEPY.name == ownerAnimal.animalFeedStatus) {
                    Log.runtime(TAG, "小鸡厨房🐔[小鸡正在睡觉中，跳过厨房功能]")
                } else {
            ...
            if (AnimalInteractStatus.HOME.name != ownerAnimal.animalInteractStatus) { //如果小鸡不在家
                if ("ORCHARD" == ownerAnimal.locationType) {
                    ...
                    when (SubAnimalType.valueOf(ownerAnimal.subAnimalType!!)) {
                        SubAnimalType.GUEST -> {
                            guest = true
                            Log.runtime(TAG, "小鸡到好友家去做客了")
                        }

                        SubAnimalType.NORMAL -> Log.runtime(TAG, "小鸡太饿，离家出走了")
                        SubAnimalType.PIRATE -> Log.runtime(TAG, "小鸡外出探险了")
                        SubAnimalType.WORK -> Log.runtime(TAG, "小鸡出去工作啦")
                    }
                    var hungry = false
                    val userName =
                    ...
                    when (AnimalFeedStatus.valueOf(ownerAnimal.animalFeedStatus!!)) {
                        AnimalFeedStatus.HUNGRY -> {
                            hungry = true
                            Log.runtime(TAG, "小鸡在[$userName]的庄园里挨饿")
                        }

                        AnimalFeedStatus.EATING -> Log.runtime(
                            TAG,
                            "小鸡在[$userName]的庄园里吃得津津有味"
                        )
                        AnimalFeedStatus.SLEEPY -> Log.runtime(TAG, "小鸡在[$userName]的庄园里睡觉")
                        AnimalFeedStatus.NONE -> Log.runtime(TAG, "小鸡在[$userName]的庄园里状态未知")
                    }
    ...
    private suspend fun paradiseCoinExchangeBenefit() {
        try {
            ...
            if (!ResChecker.checkRes("$TAG[mallHome]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "paradiseCoinExchangeBenefit 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun exchangeBenefit(spuId: String?): Boolean {
        try {
            ...
            if (!ResChecker.checkRes("$TAG[mallItemDetail]", jo)) {
                return false
            }
    ...
    private fun exchangeBenefit(spuId: String?, skuId: String?): Boolean {
        try {
            ...
            return ResChecker.checkRes("$TAG[buyMallItem]", jo)
        } catch (t: Throwable) {
            Log.printStackTrace(TAG, "exchangeBenefit err:",t)
        }
        return false
    ...
    private fun enterFarm(): JSONObject? {
        try {
            ...
            if (ResChecker.checkRes("$TAG[enterFarm]", jo)) {
                rewardProductNum =
...
//        }
        ...
        if (useBigEaterTool!!.value && AnimalFeedStatus.EATING.name == ownerAnimal.animalFeedStatus) {
            ...
            if (serverUseBigEaterTool) {
                ...
                if (usedCount >= 2) {
                    Log.runtime("今日加饭卡已使用${usedCount}/2，跳过使用")
                } else {
                    ...
                    if (result) {
                        ...
                        Log.runtime("⚠️使用道具🎭[加饭卡]失败，可能卡片不足或状态异常~")
                    }
                }
            }
        }
...
//                                        Log.runtime(TAG, "🔄 下一次蹲点任务已创建")
                        ...
                        Log.runtime(UserMap.getCurrentMaskName() + "小鸡的蹲点投喂时间[" + TimeUtil.getCommonDate(nextFeedTime)+"]")
                    } else {
                        Log.runtime(TAG, "蹲点投喂🥣[倒计时为0，开始投喂]")
                        if (feedAnimal(ownerFarmId)) {
                            // 刷新状态
                            syncAnimalStatus(ownerFarmId)
                        }
            ...
            Log.runtime(TAG, "蹲点投喂🥣[小鸡正在睡觉，暂不安排投喂任务]")
        }
    ...
    private fun animalSleepNow() {
        try {
            ...
            var jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[loveCabin]", jo)) {
                ...
                if (sleepNotifyInfo.optBoolean("canSleep", false)) {
                    ...
                    jo = JSONObject(s)
                    if (ResChecker.checkRes("$TAG[sleep]", jo)) {
    ...
    private fun animalWakeUpNow() {
        try {
            var s = AntFarmRpcCall.queryLoveCabin(UserMap.currentUid)
            var jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[loveCabinWake]", jo)) {
                val sleepNotifyInfo = jo.getJSONObject("sleepNotifyInfo")
                if (!sleepNotifyInfo.optBoolean("canSleep", true)) {
                    s = AntFarmRpcCall.wakeUp()
                    jo = JSONObject(s)
                    if (ResChecker.checkRes("$TAG[wakeUp]", jo)) {
                        Log.farm("小鸡起床 🛏")
                    }
                } else {
                    Log.farm("小鸡无需起床 🛏")
        ...
        try {
            if (rewardList != null) {
                for (rewardFriend in rewardList) {
                    ...
                    if (ResChecker.checkRes("$TAG[rewardFriend]", jo)) {
    ...
    private fun recallAnimal(
        ...
        try {
            ...
            val jo = JSONObject(s)
            val memo = jo.getString("memo")
            if (ResChecker.checkRes("$TAG[recallAnimal]", jo)) {
        ...
        try {
            for (animal in animals) {
                if (AnimalInteractStatus.STEALING.name == animal.animalInteractStatus && (SubAnimalType.GUEST.name != animal.subAnimalType) && (SubAnimalType.WORK.name != animal.subAnimalType)) {
                    ...
                    val jo = JSONObject(s)
                    val memo = jo.getString("memo")
                    if (ResChecker.checkRes("$TAG[sendBackAnimal]", jo)) {
    ...
    private fun receiveToolTaskReward() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[listToolTask]", jo)) {
                ...
                for (i in 0..<jaList.length()) {
                    ...
                    if (joItem.has("taskStatus")
                        ...
                        if (isFull) {
                            Log.runtime(TAG, "领取道具[" + toolType.nickName() + "]#已满，暂不领取")
                            continue
                        }
                        ...
                        if (ResChecker.checkRes("$TAG[receiveToolReward]", jo)) {
    ...
    private fun harvestProduce(farmId: String?) {
        try {
            ...
            val jo = JSONObject(s)
            val memo = jo.getString("memo")
            if (ResChecker.checkRes("$TAG[harvestProduce]", jo)) {
    ...
    private fun answerQuestion(activityId: String?) {
        try {
            ...
            // 缓存未命中时调用AI
            if (!cacheHit) {
                Log.runtime(TAG, "缓存未命中，尝试使用AI答题：$title")
...
//        Log.runtime(TAG, "convertDateToInt 开始转换日期：$dateStr")
    ...
    private suspend fun recordFarmGame(gameType: GameType) {
        try {
            while (true) {
                ...
                if (!ResChecker.checkRes("$TAG[initFarmGame]", joInit)) break

                val gameAward = joInit.optJSONObject("gameAward")
                if (gameAward?.optBoolean("level3Get") == true) {
                    Log.runtime(TAG, "庄园游戏🎮[${gameType.gameName()}]#今日奖励已领满")
                    break
                }
                ...
                if (remainingCount > 0) {
                    ...
                    if (ResChecker.checkRes("$TAG[recordFarmGame]", joRecord)) {
                        ...
                        Log.runtime(TAG, "庄园游戏提交失败: $joRecord")
                    }
                }
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "recordFarmGame 协程被取消")
            throw e
        } catch (t: Throwable) {
        ...
        when {
            ...
            ignoreAcceLimitMode -> {
                if (isInsideTimeRange) {
                    ...
                    Log.runtime("当前处于按时游戏改分模式，未到设定时间，跳过")
                }
            }
            ...
            isAccelEnabled && (isAccelLimitReached || accelerateToolCount <= 0) -> {
                ...
                when {
                    ...
                    !isTaskEnabled -> {
                        Log.runtime("未开启饲料任务，虽然尝试领取了奖励，但饲料缺口仍超过${gameRewardMax!!.value}g，直接执行游戏")
                        playAllFarmGames()
                    }

                    isTaskFinished -> {
                        Log.runtime("已开启饲料任务且今日已完成，但领取奖励后缺口仍超过${gameRewardMax!!.value}g，暂不执行游戏改分。" +
                                "请确认饲料奖励完成情况，可以关闭设置里的“做饲料任务”选项直接进行游戏改分")
                    }

                    else -> {
                        Log.runtime("已开启饲料任务但尚未完成，现有饲料缺口超过${gameRewardMax!!.value}g，等待任务完成后再执行")
                    }
                }
            }
            ...
            isAccelEnabled && accelerateToolCount > 0 -> {
                Log.runtime("加速卡有${accelerateToolCount}张，已使用${Status.INSTANCE.useAccelerateToolCount}张，" +
        ...
        when {
            ignoreAcceLimitMode -> {
                if (isTimeReached) {
                    ...
                    Log.runtime(TAG, "当前处于按时抽抽乐模式，未到设定时间，跳过")
                }
            }
    ...
    private suspend fun doFarmTasks() {
        try {
            ...
            if (!ResChecker.checkRes("$TAG[listFarmTask]", jo)) return
...
//                    Log.runtime(TAG, "跳过黑名单任务: $title ($bizKey)")
                    continue
                }
                ...
                when (taskStatus) {
                    TaskStatus.TODO.name -> {
                        when (bizKey) {
                            "VIDEO_TASK" -> {
                                // --- 视频任务专项逻辑 ---
                                Log.runtime(TAG, "开始处理视频任务: $title ($bizKey)")
                    ...
                    TaskStatus.FINISHED.name, TaskStatus.RECEIVED.name -> {
                        ...
                        Log.runtime(TAG, "跳过非TODO任务: $title ($bizKey) 状态: $taskStatus")
                    }
                }
    ...
    private suspend fun handleVideoTask(bizKey: String, title: String) {
        ...
        if (ResChecker.checkRes("$TAG[videoUrl]", jo)) {
            ...
            try {
                ...
                if (ResChecker.checkRes("$TAG[videoDeliver]", JSONObject(AntFarmRpcCall.videoDeliverModule(contentId)))) {
                    delay(15000L) // 模拟观看视频
                    if (ResChecker.checkRes("$TAG[videoTrigger]", JSONObject(AntFarmRpcCall.videoTrigger(contentId)))) {
    ...
    private fun handleGeneralTask(bizKey: String, title: String) {
        ...
        if (ResChecker.checkRes("$TAG[doFarmTask]", jo)) {
            Log.farm("庄园任务完成🧾[$title]")
        } else {
            val resultCode = jo.optString("resultCode", "")
            if (resultCode == "309") {
                Status.setFlagToday("farm::task::limit::$bizKey")
                Log.runtime(TAG, "庄园任务[$title]已达上限")
            } else {
                val errorMsg = when {
    ...
    private suspend fun receiveFarmAwards() {
        try {
                ...
                if (ResChecker.checkRes(TAG + "查询庄园任务失败:", jo)) {
                    ...
                    if (!Status.hasFlagToday("farm::signed")) {
                        ...
                        if (shouldSign) {
                            ...
                            Log.runtime(TAG, "${msg}。14点后会强制签到；如已签到请忽略")
                        }
                    }
                        ...
                        if (TaskStatus.FINISHED.name == taskStatus) {
                            ...
                            if ("ALLPURPOSE" == task.optString("awardType")) {
                                ...
                                if (!ignoreAcceLimit!!.value && (needFarmGame && foodStock >= (foodStockLimit - gameRewardMax!!.value))) {
                                    unreceiveTaskAward++
                                    Log.runtime("当日游戏改分未完成，预留最多${gameRewardMax!!.value}饲料空间，现有饲料${foodStock}g")
                                    isFeedFull = true
                                    break
                                }
                                if (awardCount > foodStockLeft) {
                                    if (!isNight) {
                                        ...
                                        Log.runtime(TAG, "领取任务：${taskTitle} 的饲料奖励 ${awardCount}g后将超过[${foodStockLimit}g]上限!终止领取。现有饲料${foodStock}g")
                                        unreceiveTaskAward++
                                        isFeedFull = true
                                        break
                                    } else {
                                        Log.runtime("20点后领取任务：${taskTitle} 的饲料奖励 ${awardCount}g后饲料将超过上限，现有饲料${foodStock}g，溢出${awardCount - foodStockLeft}g")
                                    }
                                }
                            }
                            ...
                            if (ResChecker.checkRes(TAG + "领取庄园任务奖励失败:", receiveTaskAwardjo)) {
                                ...
                                if ("331" == resultCode || memo.contains("饲料槽已满")) {
                                    Log.runtime(TAG, "领取失败：饲料槽已满，停止后续领取")
                                    isFeedFull = true
                                    break
                                } else {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "receiveFarmAwards 协程被取消")
            throw e
        } catch (t: Throwable) {
            ...
            for (i in 0..<jaFarmSignList.length()) {
                ...
                if (currentSignKey == signKey) {
                    if (!signed) {
                        ...
                        if (ResChecker.checkRes("$TAG[farmSign]", signResponse)) {
            ...
            if (foodStock < 180) {
                ...
                if (ResChecker.checkRes("$TAG[feedAnimal]", jo)) {
                    ...
                    if (timeSendBack!!.value in 10..interval){
                        ...
                        Log.runtime(TAG, "设置个合理的喂食后赶鸡时间，建议 30 分钟")
                    }
                    if (sendBackAnimal!!.value && timeSendBackAnimal > 0) {
                        try {
                            ...
                            addChildTask(task)
                            Log.runtime(UserMap.getCurrentMaskName() + "${timeSendBackAnimal}分钟后${kcTime}蹲点赶小鸡")
    ...
    private fun listFarmTool(): List<FarmTool>? {
        try {
            ...
            if (ResChecker.checkRes("$TAG[listFarmTool]", jo)) {
    ...
    private suspend fun useAccelerateTool(): Boolean {
        ...
        while (remainingFood >= remainingTimeValue / 60.0 * foodConsumePerHour ) {
            // 检查本地计数器上限，防止无限使用
            if (!Status.canUseAccelerateTool()) {
                Log.runtime(TAG, "加速卡内部⏩已达到本地使用上限(8次)，停止使用")
            ...
            if (useFarmTool(ownerFarmId, ToolType.ACCELERATETOOL)) {
                ...
                Log.runtime(TAG, "加速卡内部⏩useFarmTool 返回失败，终止循环")
        ...
        // 这里打印没有连续使用8张加速卡的原因
        when(exitReason){
            "CONDITION_NOT_MET" -> Log.runtime("剩余可加速的时间少于设置的${remainingTimeValue}分钟，将在下次喂食后再次使用加速卡")
            "SINGLE_USE_MODE" -> Log.runtime("开启了“仅在满状态使用加速卡")
            "EMOTION_NOT_MAX" -> Log.runtime("开启了“仅心情满值时加速”，且当前心情不为 100")
        }
        Log.runtime(TAG, "加速卡内部⏩最终 isUseAccelerateTool=$isUseAccelerateTool")
    ...
    private fun useFarmTool(targetFarmId: String?, toolType: ToolType): Boolean {
        try {
            ...
            var jo = JSONObject(s)
            var memo = jo.getString("memo")
            if (ResChecker.checkRes("$TAG[listFarmToolUse]", jo)) {
                val jaToolList = jo.getJSONArray("toolList")
                for (i in 0..<jaToolList.length()) {
                    jo = jaToolList.getJSONObject(i)
                    if (toolType.name == jo.getString("toolType")) {
                        ...
                        if (toolCount > 0) {
                            if (toolType == ToolType.FENCETOOL && hasFence) {
                                Log.runtime(TAG, "🛡️ 篱笆效果尚在（剩余${fenceCountDown/60}分钟），跳过重复使用")
                                return false
                            }
                            var toolId = ""
                            ...
                            jo = JSONObject(s)
                            memo = jo.getString("memo")
                            if (ResChecker.checkRes("$TAG[useFarmTool]", jo)) {
    ...
    private suspend fun feedFriend() {
        try {
            ...
            for (entry in feedFriendAnimalMap.entries) {
                ...
                val jo = JSONObject(AntFarmRpcCall.enterFarm(userId, userId))
                delay(3 * 1000L) //延迟3秒
                if (ResChecker.checkRes("$TAG[enterFarmFeedFriend]", jo)) {
                        ...
                        if (masterFarmId == friendFarmId) { //遍历到的鸡 如果在自己的庄园
                            ...
                            if (AnimalInteractStatus.HOME.name == animalInteractStatus && AnimalFeedStatus.HUNGRY.name == animalFeedStatus) { //状态是饥饿 并且在庄园
                                ...
                                if (foodStock >= 180) {
                                    ...
                                    if (ResChecker.checkRes("$TAG[feedFriendAnimal]", feedFriendAnimaljo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "feedFriend 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun notifyFriend() {
        ...
        try {
            ...
            do {
                ...
                jo = JSONObject(s)
                var memo = jo.getString("memo")
                if (ResChecker.checkRes("$TAG[rankingList]", jo)) {
                    ...
                    for (i in 0..<jaRankingList.length()) {
                        ...
                        if (jo.getBoolean("stealingAnimal") && !starve) {
                            ...
                            jo = JSONObject(s)
                            memo = jo.getString("memo")
                            if (ResChecker.checkRes("$TAG[enterFarmNotify]", jo)) {
                                jo = jo.getJSONObject("farmVO").getJSONObject("subFarmVO")
            ...
            Log.runtime(TAG, "饲料剩余[" + foodStock + "g]")
        } catch (t: Throwable) {
    ...
    private fun notifyFriend(
        ...
        try {
            if (AnimalInteractStatus.STEALING.name == joAnimalStatusVO.getString("animalInteractStatus") && AnimalFeedStatus.EATING.name == joAnimalStatusVO.getString(
                ...
                if (ResChecker.checkRes("$TAG[notifyFriend]", jo)) {
        ...
        try {
            ...
            if (subFarmVO.has("manureVO")) { //粪肥 鸡屎
                ...
                for (i in 0..<manurePotList.length()) {
                    ...
                    if (manurePot.getInt("manurePotNum") >= 100) { //粪肥数量
                        ...
                        if (ResChecker.checkRes("$TAG[collectManurePot]", joManurePot)) {
            ...
            if (jo.has("buffInfoVO")) {
                ...
                if (buffType == "FENCE") {
                    ...
                    if (hasFence) {
                        Log.runtime(TAG, "🛡️ 篱笆生效中，剩余时间: ${fenceCountDown / 3600}小时${(fenceCountDown % 3600) / 60}分")
                    }
                }
            } else {
                hasFence = false
            ...
            for (i in 0..<jaAnimals.length()) {
                ...
                //                Log.runtime(TAG, "当前动物：" + animal.toString());
            }
    ...
    private fun collectDailyFoodMaterial() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[enterKitchen]", jo)) {
                ...
                if (jo.has("orchardFoodMaterialStatus")) {
                    ...
                    if ("FINISHED" == orchardFoodMaterialStatus.optString("foodStatus")) {
                        ...
                        if (ResChecker.checkRes("$TAG[foodMaterialCollect]", jo)) {
    ...
    private fun collectDailyLimitedFoodMaterial() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[foodMaterialPack]", jo)) {
                ...
                if (canCollectDailyLimitedFoodMaterial) {
                    ...
                    if (ResChecker.checkRes("$TAG[collectLimitedFood]", jo)) {
    ...
    private suspend fun cook() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[enterKitchenCook]", jo)) {
                ...
                if (cookTimesAllowed > 0) {
                    for (i in 0..<cookTimesAllowed) {
                        ...
                        if (ResChecker.checkRes("$TAG[cook]", jo)) {
    ...
    private fun useSpecialFood(cuisineList: JSONArray, maxUsage: Int = cuisineList.length()) {
        try {
            ...
            Log.runtime(TAG, "美食总量为:$totalFoodCount")
            ...
            while (totalUsed < maxUsage) {
                ...
                for (i in 0..<cuisineList.length()) {
                    ...
                    if (ResChecker.checkRes("$TAG[useFarmFood]", joRes)) {
    ...
    private suspend fun visitFriend(userId: String?, count: Int): Int {
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[enterFarmVisit]", jo)) {
                ...
                for (i in 0..<count) {
                    ...
                    if (ResChecker.checkRes("$TAG[visitFriend]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "visitFriend 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun acceptGift() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[acceptGift]", jo)) {
    ...
    private fun diaryTietze(@Suppress("SameParameterValue") queryDayStr: String?) {
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[queryChickenDiary]", jo)) {
                ...
                if (data.has("hasTietie")) {
                    if (!data.optBoolean("hasTietie", true)) {
                        ...
                        if (ResChecker.checkRes("$TAG[diaryTietie]", jo)) {
                        ...
                        if (statisticsList.length() > 0) {
                            for (i in 0..<statisticsList.length()) {
                                ...
                                if (ResChecker.checkRes("$TAG[diaryTietieList]", jo)) {
                                    val prizeType = jo.getString("prizeType")
                                    val prizeNum = jo.optInt("prizeNum", 0)
                                    Log.farm("[$diaryDateStr]贴贴小鸡💞[$prizeType*$prizeNum]")
                                } else {
    ...
    private fun collectChickenDiary(queryDayStr: String?): String? {
        ...
        try {
            var jo = JSONObject(AntFarmRpcCall.queryChickenDiary(queryDayStr))
            if (ResChecker.checkRes("$TAG[queryDiaryCollect]", jo)) {
                val data = jo.getJSONObject("data")
                val chickenDiary = data.getJSONObject("chickenDiary")
                diaryDateStr = chickenDiary.getString("diaryDateStr")
                // 点赞小鸡日记
    ...
    private suspend fun queryChickenDiaryList(
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[diaryList]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "queryChickenDiaryList 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private suspend fun doChickenDiary() {
        ...
        try {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "doChickenDiary 协程被取消")
            throw e
        } catch (e: Exception) {
    ...
    private fun visitAnimal() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[visitAnimal]", jo)) {
                ...
                if (ResChecker.checkRes("$TAG[friendAnimalVisit]", jo)) {
                    for (i in 0..<talkNodes.length()) {
                        ...
                        if (ResChecker.checkRes("$TAG[animalSendPrize]", jo)) {
    ...
    private  fun hireAnimal() {
        ...
        try {
            ...
            if ("SUCCESS" == jsonObject.getString("memo")) {
                ...
                Log.runtime(jsonObject.getString("memo"))
            }
        } catch (t: Throwable) {
        ...
        try {
            ...
            // 前置检查：饲料是否足够
            if (foodStock < 50) {
                Log.runtime(TAG, "❌ 雇佣失败：饲料不足（当前${foodStock}g，至少需要50g）")
                return
            }
            ...
            if (hireAnimalSet.isEmpty()) {
                Log.runtime(TAG, "❌ 雇佣失败：未配置雇佣好友列表")
                Toast.show(
                    "⚠️ 雇佣小鸡配置错误\n" +
                            "已开启「雇佣小鸡」但未配置好友列表\n" +
                            "请在「雇佣小鸡 | 好友列表」中勾选好友"
            ...
            do {
                ...
                jo = JSONObject(s)
                val memo = jo.getString("memo")
                if (ResChecker.checkRes("$TAG[rankingListHire]", jo)) {
                    hasNext = jo.getBoolean("hasNext")
                    val jaRankingList = jo.getJSONArray("rankingList")
                    pageStartSum += jaRankingList.length()
                    for (i in 0..<jaRankingList.length()) {
            ...
            if (animalCount < 3) {
                ...
                Log.runtime(TAG, "雇佣小鸡结果统计：")
                Log.runtime(TAG, "  • 成功雇佣：${hiredCount}只")
                Log.runtime(TAG, "  • 还需雇佣：${stillNeed}只")
                Log.runtime(TAG, "  • 已检查好友：${checkedCount}人")
                Log.runtime(TAG, "  • 可雇佣状态：${availableCount}人")

                if (availableCount == 0) {
                    Log.runtime(TAG, "❌ 失败原因：好友列表中没有可雇佣的小鸡")
                    Log.runtime(TAG, "   建议：等待好友的小鸡回家或添加更多好友")
                } else if (hiredCount < availableCount) {
                    Log.runtime(TAG, "⚠️ 部分雇佣失败：好友的小鸡可能不在家")
                } else {
                    Log.runtime(TAG, "❌ 失败原因：可雇佣的小鸡数量不足")
                }
            } else if (hiredCount > 0) {
                Log.runtime(TAG, "✅ 雇佣成功：共雇佣${hiredCount}只小鸡")
            }
        } catch (t: Throwable) {
    ...
    private fun hireAnimalAction(userId: String?): Boolean {
        try {
            ...
            var jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[enterFarmHire]", jo)) {
                val farmVO = jo.getJSONObject("farmVO")
                val subFarmVO = farmVO.getJSONObject("subFarmVO")
                val farmId = subFarmVO.getString("farmId")
    ...
    private suspend fun handleNpcAnimalLogic() {
        try {
            ...
            if (ResChecker.checkRes("$TAG[syncNpc]", joSync)) {
            ...
            if (currentNpcAnimal == null) {
                ...
                Log.runtime(TAG, "NPC小鸡🤖[当前未雇佣，准备雇佣${targetConfig.nickName}]")
                ...
                if (currentId == targetConfig.animalId) {
                    ...
                    Log.runtime(TAG, "NPC小鸡🤖[检测到${currentName}，目标是${targetConfig.nickName}，执行切换]")
                    ...
                    if (ResChecker.checkRes("$TAG[sendBackNpc]", JSONObject(sendBackRes))) {
                        Log.farm("NPC小鸡🤖[已遣返${currentName}]")
                        delay(1500)
                        // 雇佣新的
                        hireNpc(targetConfig)
                    } else {
                        Log.runtime(TAG, "NPC小鸡🤖[遣返失败，暂停切换]")
                    }
                }
            }
        } catch (t: Throwable) {
    ...
    private fun hireNpc(config: NpcConfig): Boolean {
        try {
            ...
            val jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[hireNpc]", jo)) {
                Log.farm("NPC小鸡🤖[成功雇佣${config.nickName}]")
                syncAnimalStatus(ownerFarmId) // 刷新状态
                return true
            } else {
                Log.runtime(TAG, "NPC小鸡🤖[雇佣${config.nickName}失败: ${jo.optString("memo")}]")
            }
        } catch (e: Exception) {
    ...
    private suspend fun checkRewardAndTask(animal: Animal, animalJson: JSONObject?, config: NpcConfig) {
        ...
        if (isFull) {
            ...
            if (ResChecker.checkRes("$TAG[sendBackNpcReward]", joSendBack)) {
                Log.farm("NPC小鸡🤖[产出奖励领取成功]")
                delay(2000)
                if (!hireNpc(config)) {
                    Log.runtime(TAG, "NPC小鸡🤖[重雇失败，请检查状态]")
                }
            } else {
                Log.runtime(TAG, "NPC小鸡🤖[遣返领取奖励失败: ${joSendBack.optString("memo")}]")
            }
        } else {
            Log.runtime(TAG, "NPC小鸡🤖[${config.nickName}工作中... 当前产出:$currentReward]")
        }
    }
    ...
    private fun handleZhimaPigeonTasks() {
        try {
            ...
            val jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[listZhimaTask]", jo)) {
                ...
                for (i in 0 until taskList.length()) {
                    ...
                    if (TaskStatus.FINISHED.name == taskStatus) {
                        ...
                        if (ResChecker.checkRes("$TAG[receiveZhimaAward]", awardJo)) {
                            val awardCount = task.optInt("awardCount", 0)
    ...
    private suspend fun handleGoldChickenTasks() { // 🟢 修改点：增加 suspend 关键字
        try {
            ...
            val jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[listGoldTask]", jo)) {
                val taskList = jo.optJSONArray("farmTaskList") ?: return
                for (i in 0 until taskList.length()) {
                    val task = taskList.getJSONObject(i)
                    val taskId = task.optString("taskId")
                    ...
                    if (TaskStatus.FINISHED.name == taskStatus) {
                        ...
                        val awardJo = JSONObject(awardRes)
                        if (ResChecker.checkRes("$TAG[receiveGoldAward]", awardJo)) {
                            val awardCount = task.optInt("awardCount", 0)
                    ...
                    // 2. 做任务 (仅处理 TRIGGER 类型，如"开始攒黄金"、"领体验金")
                        ...
                        if ("CAIFU_NPC_FUND_TASK" == bizKey) {
                            ...
                            Log.runtime(TAG, "NPC任务🤖[处理体验金任务: $title]")
                            // 1. 查询首页
                            ...
                            Log.runtime(TAG, "NPC任务🤖[浏览攒黄金页面: $title]")
    ...
    private fun handleFarmChickenTasks() {
        try {
            ...
            val jo = JSONObject(s)
            if (ResChecker.checkRes("$TAG[listFarmChickenTask]", jo)) {
                val taskList = jo.optJSONArray("farmTaskList") ?: return
                for (i in 0 until taskList.length()) {
                    val task = taskList.getJSONObject(i)
                    val taskId = task.optString("taskId")
                    ...
                    if (TaskStatus.FINISHED.name == taskStatus) {
                        ...
                        val awardJo = JSONObject(awardRes)
                        if (ResChecker.checkRes("$TAG[receiveFarmChickenAward]", awardJo)) {
                            val awardCount = task.optInt("awardCount", 0)
        ...
        try {
            ...
            while (loopCount < 3) {
                ...
                if (!jo.optBoolean("success")) {
                    Log.runtime(TAG, "queryGameList 失败: $jo")
                    return
                }
                ...
                if (quotaCanUse > 0) {
                    Log.runtime(TAG, "当前有 $quotaCanUse 个宝箱待开启...")
                    while (quotaCanUse > 0) {
        ...
        try {
            ...
            for (i in 0 until taskList.length()) {
                ...
                if (taskStatus == "TODO" && taskType == "2026cc_lyqd") {
                    ...
                    Log.runtime(TAG, "正在尝试完成庄园乐园每日签到...")
                    ...
                    if (finishJo.optBoolean("success")) {
                        Log.runtime(TAG, "庄园乐园每日签到完成成功")
                        taskStatus = "FINISHED"
                    } else {
                        Log.runtime(TAG, "庄园乐园每日签到完成失败: ${finishJo.optString("desc")}")
                    }
                    delay(2000)
                }

                if (taskStatus == "FINISHED") {
                    Log.runtime(TAG, "发现可领取的乐园任务奖励: $title, 额度: $awardCount")
                    ...
                    if (claimJo.optBoolean("success")) {
                        ...
                        Log.runtime(TAG, "领取乐园任务奖励失败: ${claimJo.optString("desc")}")
                    }
                    delay(2000)
                }
            }
            ...
            if (jo.optBoolean("success")) {
                ...
                if (getFeedType!!.value == GetFeedType.GIVE) {
                    for (userId in userIdList) {
                        if (invitesToSend <= 0) {
//                            Log.runtime(TAG,"已达到最大邀请次数限制，停止发送邀请。");
                            break
                        }
    ...
    suspend fun family() {
        ...
        try {
            ...
            if (!ResChecker.checkRes("$TAG[enterFamily]", jo)) return
    ...
    private fun syncFamilyStatusIntimacy(groupId: String?) {
        try {
            ...
            ResChecker.checkRes("$TAG[syncFamilyIntimacy]", jo)
        } catch (e: CancellationException) {
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "syncFamilyStatusIntimacy 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private suspend fun inviteFriendVisitFamily(friendUserIds: MutableList<String?>) {
        try {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "inviteFriendVisitFamily 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private suspend fun familyBatchInviteP2PTask(
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[familyShareP2P]", jo)) {
                ...
                if (ResChecker.checkRes("$TAG[familyBatchInvite]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyBatchInviteP2PTask 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private suspend fun familyDrawTask(friendUserIds: MutableList<String?>, familyDrawInfo: JSONObject) {
        try {
            ...
            if (ResChecker.checkRes("$TAG[queryFamilyDraw]", jo)) {
                delay(1000)
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyDrawTask 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun giftFamilyDrawFragment(giftUserId: String?, giftNum: Int) {
        try {
            ...
            if (ResChecker.checkRes("$TAG[giftFamilyFragment]", jo)) {
    ...
    private fun familyDrawListFarmTask(): JSONArray? {
        try {
            ...
            if (ResChecker.checkRes("$TAG[familyDrawListTask]", jo)) {
    ...
    private fun familyDraw(): Boolean {
        try {
            ...
            if (ResChecker.checkRes("$TAG[familyDraw]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyDraw 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private suspend fun familyEatTogether(
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[familyEatTogether]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyEatTogether 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun familyDrawSignReceiveFarmTaskAward(taskId: String?, title: String?) {
        try {
            ...
            if (ResChecker.checkRes("$TAG[familyDrawSignAward]", jo)) {
                Log.farm("亲密家庭🏠扭蛋任务#$title#奖励领取成功")
            }
        } catch (e: CancellationException) {
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyDrawSignReceiveFarmTaskAward 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private fun queryRecentFarmFood(queryNum: Int): JSONArray? {
        try {
            ...
            if (!ResChecker.checkRes("$TAG[queryRecentFood]", jo)) {
                return null
            }
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "queryRecentFarmFood 协程被取消")
            throw e
        } catch (t: Throwable) {
        ...
        try {
            for (i in 0..<animals.length()) {
                ...
                if (AnimalInteractStatus.HOME.name == animalStatusVo.getString("animalInteractStatus") && AnimalFeedStatus.HUNGRY.name == animalStatusVo.getString(
                    ...
                    if (ResChecker.checkRes("$TAG[familyFeedFriend]", jo)) {
            ...
            // 协程取消异常必须重新抛出，不能吞掉
            Log.runtime(TAG, "familyFeedFriendAnimal 协程被取消")
            throw e
        } catch (t: Throwable) {
    ...
    private  fun clickForGiftV2(gift: JSONObject?) {
        ...
        try {
            ...
            if (ResChecker.checkRes("$TAG[clickGift]", resultJson)) {
    ...
    fun manualSendBackAnimal() {
        try {
            Log.runtime(TAG, "🚀 开始执行手动遣返小鸡任务...")
            // 必须先进入农场获取最新 animal 数据
            if (enterFarm() != null) {
                sendBackAnimal()
                Log.runtime(TAG, "✅ 手动遣返指令执行完毕")
            } else {
                Log.runtime(TAG, "❌ 进入农场失败，无法执行遣返")
            }
        } catch (t: Throwable) {
    ...
    fun manualChouChouLeLogic() {
        try {
            Log.runtime(TAG, "🚀 开始执行手动抽抽乐任务...")
            if (enterFarm() != null) {
                playChouChouLe()
                Log.runtime(TAG, "✅ 手动抽抽乐任务处理完毕")
            }
        } catch (t: Throwable) {
    ...
    fun manualUseSpecialFood(count: Int) {
        try {
            ...
            if (count <= 0) {
                Log.runtime(TAG, "⚠️ 手动使用特殊美食已拦截：必须指定大于0的使用次数")
                return
            }

            Log.runtime(TAG, "🚀 开始执行手动使用特殊美食任务，目标数量: $count")
            val jo = enterFarm()
            if (jo != null) {
                val cuisineList = jo.getJSONArray("cuisineList")

                if (AnimalFeedStatus.SLEEPY.name == ownerAnimal.animalFeedStatus) {
                    Log.runtime(TAG, "❌ 小鸡正在睡觉，无法使用美食")
                } else {
                    useSpecialFood(cuisineList, count)
                    Log.runtime(TAG, "✅ 手动使用特殊美食任务处理完毕")
                }
            } else {
                Log.runtime(TAG, "❌ 进入农场失败，无法执行任务")
            }
        } catch (t: Throwable) {
    ...
    fun manualUseFarmTool(toolType: String, toolCount: Int) {
        try {
            if (enterFarm() != null) {
                syncAnimalStatus(ownerFarmId)
                Log.runtime(TAG, "开始执行手动使用道具: $toolType, 计划数量: $toolCount")
                val farmTools = listFarmTool()
                if (farmTools == null || farmTools.isEmpty()) {
                    Log.runtime(TAG, "❌ 获取道具列表失败或道具库为空")
                    return
                }

                val tool = farmTools.find { it.toolType?.name == toolType }
                if (tool == null) {
                    Log.runtime(TAG, "❌ 道具库中没有道具: $toolType")
                    return
                }
                if (toolType == "FENCETOOL" && hasFence) {
                    Log.runtime(TAG, "❌ 手动执行拦截：篱笆卡效果正在生效中")
                    return
                }

                Log.runtime(TAG, "当前道具 [${tool.toolType?.nickName()}] 余量: ${tool.toolCount}")

                val actualCount = if (toolType == "NEWEGGTOOL") {
                    if (tool.toolCount < toolCount) {
                        Log.runtime(TAG, "⚠️ 道具余量不足，将用完剩余的 ${tool.toolCount} 个")
                        tool.toolCount
                    } else {
                        toolCount
                    }
            ...
            Log.runtime(TAG, "❌ manualUseFarmTool 出错: ${t.message}")
            Log.printStackTrace(t)
        }
    }
}
