...
public class Finedu {
    ...
    public void handle() {
        try {
            ...
            Log.runtime(TAG, "总学分: " + afterCredits + " (增加: " + increase + " 学分)");
        } catch (NumberFormatException e) {
            Log.runtime(TAG, "学分数据格式错误");
        } catch (Exception e) {
    ...
    private void queryUserInfo() {
        try {
            ...
            Log.runtime(TAG, "用户总学分: " + totalCredits);
            ...
            if (needQueryExpiredCredit()) {
                ...
                if (expiredCreditResponse != null && expiredCreditResponse.optBoolean("success")) {
                    ...
                    if (expiredContent != null) {
                        ...
                        Log.runtime(TAG, "即将过期学分: " + willExpiredCredits + ",过期时间: " + expiredTime);
                    }
                }
            }
        } catch (Exception e) {
                ...
                try {
                    ...
                    if (!"COMPLETED".equals(status)) {
                        ...
                        if (Status.hasTemporaryStatusValid(taskErrorKey)) {
                            //Log.runtime(TAG, "任务[" + taskName + "]已在错误缓存中，跳过执行");
                            continue;
                        }
