...
object ForestUtil {
    ...
    fun isUserInFrequencyCooldown(userId: String?): Boolean {
        ...
        if (currentTime < info.cooldownUntil) {
            ...
            Log.runtime(TAG, "[${UserMap.getMaskName(userId)}] 手速太快冷却中，还需等待 ${remainingMinutes}分${remainingSeconds}秒")
            return true
        }
        // 冷却期结束，清除记录
...
//            Log.runtime(TAG, "[${UserMap.getMaskName(userId)}] 冷却期结束，恢复正常处理")
    ...
    fun recordFrequencyError(userId: String?): Boolean {
        ...
        Log.runtime(TAG, "⚠️ [$userName] 手速太快！第${info.failCount}次异常，休息${cooldownMinutes}分钟，下次暂不处理")
    ...
    fun clearAllFrequencyLimits() {
        ...
        if (count > 0) {
            ...
            Log.runtime(TAG, "已清除${count}个用户的频率限制记录")
        }
    }
    ...
    fun clearUserFrequencyLimit(userId: String?) {
        if (userId != null && userFrequencyMap.remove(userId) != null) {
            Log.runtime(TAG, "[${UserMap.getMaskName(userId)}] 频率限制已清除")
        }
    }
