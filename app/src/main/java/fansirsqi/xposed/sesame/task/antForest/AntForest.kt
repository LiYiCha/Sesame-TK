...
class AntForest : ModelTask(), EnergyCollectCallback {
    ...
    override fun check(): Boolean {
        ...
        if (forestPauseTime > currentTime) {
            Log.runtime(name + "任务-异常等待中，暂不执行检测！")
            return false
        }
        // -----------------------------
        // 3️⃣ 只收能量时间段判断
    ...
    private fun startEnergyCollectionLoop() {
        if (!isEnergyLoopRunning.compareAndSet(false, true)) {
            Log.runtime(TAG, "只收能量循环任务已在运行中，跳过重复启动。")
            return
        }
        try {
                ...
                try {
                    while (true) {
                        ...
                        if (!(TaskCommon.IS_ENERGY_TIME || hour == 7 && minute < 30)) {
                            Log.runtime(TAG, "当前不在只收能量时间段，退出循环")
                            break
                        }
                        // 收取自己能量（协程中执行）
                        Log.runtime(TAG, "🌳 开始收取自己的能量...")
                        val selfHomeObj = querySelfHome()
                        if (selfHomeObj != null) {
                            collectEnergy(UserMap.currentUid, selfHomeObj, "self")
                            Log.runtime(TAG, "✅ 收取自己的能量完成")
                        } else {
                        ...
                        Log.runtime(TAG, "✨ 只收能量时间一轮完成，等待 $sleepMillis 毫秒后开始下一轮")
    ...
    private fun createSafeIntervalLimit(
        ...
        // 记录原始输入值
        //Log.runtime(TAG, description + "原始设置值: [" + intervalStr + "]")

        // 使用自定义区间限制类，处理所有边界情况
        val limit = FixedOrRangeIntervalLimit(intervalStr, defaultMin, defaultMax)
        //Log.runtime(TAG, description + "成功创建区间限制")
        return limit
    }
    ...
    override suspend fun runSuspend() {
        ...
        Log.runtime(TAG, "游戏 2060170000363691 获取到的 authCode: $authCode   Mark:$MiniMark")
        try {
            // 每次运行时检查并更新计数器
    ...
    private fun checkAndUpdateCounters() {
        ...
        if (!Status.hasFlagToday("forest::countersReset")) {
            ...
            Log.runtime(TAG, "✅ 检测到新的一天，重置森林任务计数器与频率限制")
        }
    }
            ...
            try {
                ...
                when (val bizType = wateringBubble.getString("bizType")) {
                    ...
                    "baohuhuizeng" -> collectReturnEnergy(wateringBubble)
                        ...
                        Log.runtime(TAG, "未知bizType: $bizType")
                        continue
                    }
                }
                GlobalThreadPools.sleepCompat(500L)
            } catch (e: JSONException) {
                Log.runtime(TAG, "浇水金球JSON解析错误: " + e.message)
            } catch (e: RuntimeException) {
                Log.runtime(TAG, "浇水金球处理异常: " + e.message)
            }
        }
    }
    ...
    private fun collectWater(wateringBubble: JSONObject) {
        try {
            ...
            Log.runtime(TAG, "收取浇水JSON解析错误: " + e.message)
        }
    }
    ...
    private fun collectRebornEnergy() {
        try {
            ...
            Log.runtime(TAG, "收取金球运行时异常: " + e.message)
        }
    }
    ...
    private fun collectReturnEnergy(wateringBubble: JSONObject) {
        try {
            ...
            Log.runtime(TAG, "收取金球回赠JSON解析错误: " + e.message)
        }
    }
            ...
            if (ResChecker.checkRes(TAG + "收集能量失败:", joEnergy)) {
                ...
                if (bubbles.length() > 0) {
                    ...
                    if (collected > 0) {
                        ...
                        Log.runtime(successMessage + "失败")
                    }
                } else {
                    Log.runtime(successMessage + "失败: 未找到金球信息")
                }
            } else {
                Log.runtime(successMessage + "失败:" + joEnergy.getString("resultDesc"))
        ...
        try {
            ...
            for (i in 0..<usingUserProps.length()) {
                ...
                if (ResChecker.checkRes(TAG + "收取动物派遣能量失败:", responseObj)) {
                    ...
                    Log.runtime(TAG, "收取动物能量失败: " + responseObj.getString("resultDesc"))
                    Log.runtime(response)
                }
        ...
        try {
            ...
            for (i in 0..<usingUserProps.length()) {
                ...
                if (remainEnergy > 0) {
                    ...
                    Log.runtime(TAG, "发现[$propName]有 $remainEnergy g能量待收取，尝试收取...")
                    ...
                    if (ResChecker.checkRes(TAG + "收取炸弹卡能量失败:", responseObj)) {
                        ...
                        Log.runtime(TAG, "收取炸弹卡失败: " + responseObj.getString("resultDesc"))
                    }
                }
            }
        } catch (e: Exception) {
...
//            JSONObject bag = getBag();
            ...
            for (entry in exchangeList.entries) {
                ...
                if (count == null || count <= 0) {
                    Log.runtime(TAG, "无效的count值: skuId=$skuId, count=$count")
                    continue
                }
                // 处理活力值兑换
                while (Status.canVitalityExchangeToday(skuId, count)) {
                    if (!Vitality.handleVitalityExchange(skuId)) {
                        Log.runtime(TAG, "活力值兑换失败: " + getNameById(skuId))
                        break
                    }
                    GlobalThreadPools.sleepCompat(1000L)
                }
    ...
    private fun collectEnergy(
        ...
        try {
            ...
            if (!ResChecker.checkRes(TAG + "载入用户主页失败:", userHomeObj)) {
                Log.runtime(TAG, "载入失败: " + userHomeObj.optString("resultDesc", "未知错误"))
                return userHomeObj
            }
            val serverTime = userHomeObj.optLong("now", System.currentTimeMillis())
            ...
            if (!collectEnergy!!.value || jsonCollectMap.contains(userId)) {
                Log.runtime(TAG, "[$userName] 不允许收取能量，跳过")
                return userHomeObj
            }
            ...
            if (!isSelf) {
                ...
                if (hasShield(userHomeObj, serverTime)) {
                    hasProtection = true
                    Log.runtime(TAG, "[$userName]被能量罩❤️保护着哟，跳过收取")
                }
                ...
                if (!hasProtection && hasBombCard(userHomeObj, serverTime)) {
                    ...
                    if (bombLimit > 0) {
                        ...
                        if (bubbles != null) {
                            for (i in 0 until bubbles.length()) {
                                ...
                                if (energy >= bombLimit) {
                                    bypassBomb = true
                                    Log.runtime(TAG, "[$userName] 发现大能量球($energy g) >= 炸弹阈值($bombLimit g)，无视炸弹卡强行收取！💥")
                                    break
                                }
                            }
                        }
                    ...
                    if (!bypassBomb) {
                        hasProtection = true
                        Log.runtime(TAG, "[$userName]开着炸弹卡💣，跳过收取")
                    }
                }
            }
        ...
        return when (type) {
            CollectSelfType.OVER_THRESHOLD -> {
                ...
                if (bubbleCount < threshold && canBeRobbedAgain) {
                    ...
                    if (bubbleCount < threshold && !canBeRobbedAgain) {
                        Log.runtime(TAG, "触发保底收取：能量[$bubbleCount g] < 阈值[$threshold g]，但已无法被偷，强制收取")
                    }
                    true
                }
            }
                ...
                CollectStatus.WAITING -> {
                    ...
                    if (isSelf) {
                        ...
                        if (!shouldCollectSelfBubble(bubbleCount, canBeRobbed)) {
                            // 可选：Log.runtime(TAG, "跳过等待能量[$bubbleCount g] (不满足阈值配置)")
                            continue
                        }
                    }
    ...
    private suspend fun collectRankingsCoroutine(
        ...
        try {
            ...
            for (i in 0..2) {
                ...
                if (i < 2) {
                    Log.runtime(TAG, "获取" + rankingName + "失败，" + (5 * (i + 1)) + "秒后重试")
            ...
            Log.runtime(TAG, "收取" + rankingName + "能量完成！")
        } catch (e: CancellationException) {
            // 协程被取消是正常行为，不记录错误日志
        ...
        try {
            loop@ for (attempt in 1..maxAttempts) {
                ...
                if (friendId.isNullOrBlank()) {
                    consecutiveEmpty++
                    Log.runtime(TAG, "第$attempt 次未发现有能量的好友")

                    // 连续2次没有返回ID，说明真的没了，直接结束
                    if (consecutiveEmpty >= 2) {
                        Log.runtime(TAG, "系统无可偷取目标，结束")
                        break@loop
                    }
                    // 缓冲一下重试
                    GlobalThreadPools.sleepCompat(500L)
    ...
    private fun quickcollectEnergyByTakeLook() {
        ...
        Log.runtime(TAG, "开始找能量 (无视黑名单与道具)")
    ...
    private suspend fun processFriendsEnergyCoroutine(
        ...
        try {
            ...
            if (friendList == null) {
                Log.runtime(TAG, "${sourceName}数据为空，跳过处理")
                return@withContext
            }

            if (friendList.length() == 0) {
                Log.runtime(TAG, "${sourceName}列表为空，跳过处理")
                return@withContext
            }
            ...
            Log.runtime(TAG, "✅ ${sourceName}处理完成，耗时${elapsed}ms，平均${elapsed / friendList.length()}ms/人")
    ...
    private fun processEnergyInternal(obj: JSONObject, flag: String?) {
        ...
        //  Log.runtime(TAG, "  processEnergy 开始处理用户: [" + userName + "], 类型: " + (isPk ? "PK" : "普通"));
        if (isPk) {
           ...
           // Log.runtime(TAG, "  正在查询PK好友 [$userName$userId] 的主页...")
            ...
            if (!needCollectEnergy && !needHelpProtect && !needCollectGiftBox) {
                //   Log.runtime(TAG, "    普通好友: [$userName$userId], 所有条件不满足，跳过")
                return
            }
            var userHomeObj: JSONObject? = null
                    ...
                    for (ii in 0..<giftBoxList.length()) {
                        try {
                            ...
                            if (!ResChecker.checkRes(TAG + "领取好友礼盒失败:", giftBoxResult)) {
                                Log.runtime(giftBoxResult.getString("resultDesc"))
                ...
                for (j in 0..<wateringBubbles.length()) {
                    try {
                        ...
                        if (!ResChecker.checkRes(TAG + "复活能量失败:", joProtect)) {
                            //Log.runtime(joProtect.getString("resultDesc"))
            ...
            try {
                ...
                if (!"SUCCESS".equals(resultCode, ignoreCase = true)) {
                    if ("PARAM_ILLEGAL2" == resultCode) {
                        Log.runtime(TAG, "[" + getAndCacheUserName(userId) + "]" + "能量已被收取,取消重试 错误:" + jo.getString("resultDesc"))
                        return@Runnable
                    }
                    ...
                    Log.runtime(TAG, "[" + getAndCacheUserName(userId) + "]" + jo.optString("resultDesc", ""))
                    if (tryCount < tryCountInt!!) {
                        collectEnergyEntity.setNeedRetry()
                        collectEnergy(collectEnergyEntity)
                    }
            ...
            for (i in 0..<usingUserProps.length()) {
                ...
                when (propGroup) {
                    "doubleClick" -> {
                        ...
                        Log.runtime(TAG, "$propName 剩余时间⏰：" + formatTimeDifference(doubleEndTime - System.currentTimeMillis()))
                    }

                    "stealthCard" -> {
                        stealthEndTime = userUsingProp.getLong("endTime")
                        Log.runtime(TAG, "$propName 剩余时间⏰️：" + formatTimeDifference(stealthEndTime - System.currentTimeMillis()))
                    }

                    "shield" -> {
                        shieldEndTime = userUsingProp.getLong("endTime")
                        Log.runtime(TAG, "$propName 剩余时间⏰：" + formatTimeDifference(shieldEndTime - System.currentTimeMillis()))
                    }

                    "energyBombCard" -> {
                        energyBombCardEndTime = userUsingProp.getLong("endTime")
                        Log.runtime(TAG, "$propName 剩余时间⏰：" + formatTimeDifference(energyBombCardEndTime - System.currentTimeMillis()))
                    }

                    "robExpandCard" -> {
                        val extInfo = userUsingProp.optString("extInfo")
                        robExpandCardEndTime = userUsingProp.getLong("endTime")
                        Log.runtime(TAG, "$propName 剩余时间⏰：" + formatTimeDifference(robExpandCardEndTime - System.currentTimeMillis()))
                        if (!extInfo.isEmpty()) {
                        ...
                        Log.runtime(TAG, "跳过非目标道具:$userUsingProp")
                    }
                }
            }
        } catch (th: Throwable) {
        ...
        try {
            ...
            label@ while (waterCount <= count) {
                ...
                if (rpcResponse.isEmpty()) {
                    Log.runtime(TAG, "好友浇水返回空: " + UserMap.getMaskName(userId))
                    isContinue = false
                    break
                }
                ...
                if ("1009" == errorCode) { // 访问被拒绝
                    Log.runtime(TAG, "好友浇水🚿访问被拒绝: " + UserMap.getMaskName(userId))
                    isContinue = false
                    break
                ...
                when (resultCode) {
                    ...
                    "WATERING_TIMES_LIMIT" -> {
                        Log.runtime(TAG, "好友浇水🚿今日已达上限: " + UserMap.getMaskName(userId))
                        wateredTimes = 3 // 上限假设3次
                        break@label
                    }

                    "ENERGY_INSUFFICIENT" -> {
                        Log.runtime(TAG, "好友浇水🚿" + jo.optString("resultDesc"))
                        isContinue = false
                        break@label
                    }
        ...
        try {
            ...
            while (true) {
                ...
                if (!ResChecker.checkRes(TAG + "查询森林任务失败:", jo)) {
                    Log.runtime(jo.getString("resultDesc")) // 记录失败描述
...
//            }
    ...
    private fun shouldRenewShield(shieldEnd: Long, nowMillis: Long): Boolean {
        ...
        try {
            ...
            // 可以添加分钟有效性检查
            if (minutes > 59) {
                Log.runtime(TAG, "[保护罩] 分钟数无效: $minutes, 使用默认值")
                hours = 23
                minutes = 59
            }
        } catch (e: Exception) {
            Log.runtime(TAG, "[保护罩] 解析阈值配置异常: " + e.message + ", 使用默认值")
            hours = 23
            minutes = 59
        }
        ...
        if (shieldEnd > 0 && shieldEnd < nowMillis - 365 * TimeFormatter.ONE_DAY_MS) {
            Log.runtime(TAG, "[保护罩] ⚠️ 检测到异常时间数据(${TimeUtil.getCommonDate(shieldEnd)})，跳过检查")
            return false
        }
    ...
    private fun shouldRenewEnergyBomb(bombEnd: Long, nowMillis: Long): Boolean {
        ...
        if (bombEnd > 0 && bombEnd < nowMillis - 365 * TimeFormatter.ONE_DAY_MS) {
            Log.runtime(TAG, "[炸弹卡] ⚠️ 检测到异常时间数据(${TimeUtil.getCommonDate(bombEnd)})，跳过检查")
            return false
        }
    ...
    private fun shouldRenewDoubleCard(doubleEnd: Long, nowMillis: Long): Boolean {
        ...
        if (doubleEnd > 0 && doubleEnd < nowMillis - 365 * TimeFormatter.ONE_DAY_MS) {
            Log.runtime(TAG, "[双击卡] ⚠️ 检测到异常时间数据(${TimeUtil.getCommonDate(doubleEnd)})，跳过检查")
        ...
        try {
            do {
                ...
                if (ResChecker.checkRes(TAG + "查询道具列表失败:", propListJo)) {
                    ...
                    // 如果查询道具列表失败，则记录失败的日志
                    Log.runtime(TAG, "赠送道具查询结果" + propListJo.getString("resultDesc"))
                }
                // 等待1.5秒后再继续
            } while (true)
        } catch (th: Throwable) {
    ...
    private fun queryPropList(forceRefresh: Boolean): JSONObject? {
        ...
        try {
            ...
            if (bagObject.optBoolean("success")) {
                ...
                Log.runtime(TAG, "刷新背包失败: " + bagObject.optString("resultDesc"))
            }
        } catch (th: Throwable) {
    ...
    private fun showBag() {
        ...
        try {
            ...
            logBuilder.append("==========================")
            Log.runtime(TAG, logBuilder.toString())
        } catch (e: Exception) {
    ...
    private fun usePropBag(propJsonObj: JSONObject?, needRefreshHome: Boolean = true): Boolean {
        ...
        try {
            ...
            if (isRenewable) {
                ...
                // Log.runtime(TAG, "发送检查请求: " + checkResponse);
                ...
                if ("NEED_CONFIRM_CAN_PROLONG" == status || "REPLACE" == status) {
                    // 情况1: 需要二次确认 (真正地续写)
                    Log.runtime(TAG, propName + "需要二次确认，发送确认请求...")
                    ...
                    Log.runtime(TAG, "已使用$usedPropName")

                } else {
                    // 其他所有情况都视为最终结果，通常是失败
                    // Log.runtime(TAG, "道具状态异常或使用失败12:"+ status)
                    jo = checkResponse
                }
            } else {
                // 非续用类道具，直接使用
    ...
    private fun useDoubleCard(bagObject: JSONObject) {
        try {
            ...
            if (!Status.canDoubleToday()) {
                Log.runtime(TAG, "双击卡使用条件检查: 今日次数已达上限")
                return
            }
            // 前置检查2: 校验背包数据是否有效
            if (!bagObject.optBoolean("success")) {
                Log.runtime(TAG, "背包数据异常，无法使用双击卡$bagObject")
                return
            }
            ...
            if (doubleClickProps.isEmpty()) {
                Log.runtime(TAG, "根据设置，背包中没有需要使用的双击卡")
                return
            }
            ...
            Log.runtime(TAG, "扫描到" + doubleClickProps.size + "种双击卡，将按过期顺序尝试使用...")
            ...
            if (!success) {
                Log.runtime(TAG, "所有可用的双击卡均不满足使用条件")
            }
        } catch (th: Throwable) {
        ...
        try {
            ...
            if (availableShields.isNotEmpty()) {
                ...
                for (shieldObj in availableShields) {
                    ...
                    Log.runtime(TAG, "尝试使用保护罩: $propName")
                    if (usePropBag(shieldObj)) {
                        Log.runtime(TAG, "保护罩使用成功: $propName")
                        return // 使用成功，直接退出
                    } else {
                        Log.runtime(TAG, "保护罩使用失败: $propName，尝试下一个...")
                    }
                }
            }
            // 步骤5: 未使用成功（无论是否找到）
            Log.runtime(TAG, "背包中未找到或无法使用任何可用保护罩")
    ...
    private fun useBubbleBoostCard(bag: JSONObject? = queryPropList()) {
        try {
            ...
            val selfHomeObj = querySelfHome()
            if (selfHomeObj == null) {
                Log.runtime(TAG, "无法获取自己主页信息，跳过使用加速卡")
                return
            }
            ...
            if (!hasWaitingBubbles) {
                Log.runtime(TAG, "自己当前没有未来才会成熟的能量球，不使用加速卡")
                return
            }
        ...
        try {
            ...
            if (selfHomeObj != null) {
                ...
                if (availableBubbles.isNotEmpty()) {
                    Log.runtime(TAG, "🎯 $tag：找到${availableBubbles.size}个符合阈值条件的可收能量球")
                    ...
                    Log.runtime(TAG, "🎯 $tag：未找到满足条件的能量球 (可能是被阈值过滤或无能量)")
                }
            }
        } catch (e: Exception) {
        ...
        try {
            ...
            for (propType in propTypes) {
                while (true) {
                    ...
                    if (usePropBag(jo)) {
                        Log.runtime(TAG, "成功使用一个能量雨道具: $propType")
                        delay(2000)
                    } else {
                        break
                    }
    ...
    fun doforestgame() {
        try {
            ...
            if (drawRights != null) {
                ...
                //Log.runtime(TAG, "游戏中心状态: 待开 $canUseCount 个, 已得 $usedCount/$limitCount")
        ...
        try {
            ...
            if (propObj != null) {
                ...
                if ("双击卡" == config.propName) {
                    val propType = propObj.optString("propType")
                    if ("ENERGY_DOUBLE_CLICK" == propType && !hasDoubleCardTime()) {
                        Log.runtime(TAG, "跳过双击卡[$propType]，当前不在指定使用时间段内")
                        return
                    }
                }
    ...
    private fun checkUserShieldAndBomb(userHomeObj: JSONObject, userName: String?, userId: String, serverTime: Long): Boolean {
        ...
        if (!isSelf) {
            ...
            if (shieldEndTime > serverTime) {
                ...
                Log.runtime(TAG, "[$userName]被能量罩❤️保护着哟(还剩${remainingHours}h)，跳过收取")
            }
            if (bombEndTime > serverTime) {
                hasProtection = true
                val remainingHours = (bombEndTime - serverTime) / (1000 * 60 * 60)
                Log.runtime(TAG, "[$userName]开着炸弹卡💣(还剩${remainingHours}h)，跳过收取")
            }
        }
    ...
    private fun collectEnergyForWaiting(
        ...
        try {
            Log.runtime(TAG, "蹲点收取开始：用户[${userName}] userId[${userId}] fromTag[${fromTag}]")
            // 获取服务器时间
            val serverTime = userHomeObj.optLong("now", System.currentTimeMillis())
            // 判断是否是自己的账号
            val isSelf = userId == UserMap.currentUid
            ...
            // 主号的保护罩不影响收取自己的能量
            if (isSelf && hasProtection) {
                Log.runtime(TAG, "  ⭐ 主号有保护罩，但可以收取自己的能量")
            }
            ...
            Log.runtime(TAG, "蹲点收取找到${availableBubbles.size}个可收取能量球: $availableBubbles")
        ...
        return try {
            withContext(Dispatchers.Default) {
                ...
                if (friendHomeObj != null) {
                    ...
                    Log.runtime(TAG, "蹲点收取：用户[${realUserName}] userId=${task.userId} currentUid=${UserMap.currentUid} isSelf=${isSelf}")
            ...
            // 协程取消是正常现象，不记录为错误
            Log.runtime(TAG, "collectUserEnergyForWaiting 协程被取消")
    ...
    suspend fun manualUseEnergyRain(exchange: Boolean) {
        try {
            Log.runtime(TAG, "🚀 开始执行手动能量雨任务...")
            val obj = querySelfHome()
            if (obj != null) {

                if (exchange) {
                    Log.runtime(TAG, "尝试兑换并激活能量雨卡...")
                    useEnergyRainChanceCard()
                }

                EnergyRainCoroutine.execEnergyRainCompat()
                Log.runtime(TAG, "✅ 手动能量雨任务处理完毕")
            } else {
                Log.runtime(TAG, "无法获取自己主页信息")
            }
        } catch (t: Throwable) {
