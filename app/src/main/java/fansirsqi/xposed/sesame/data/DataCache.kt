...
//            Log.runtime(TAG, "【CLEANUP】处理 key: $key, value type: ${value.javaClass}")
    ...
    fun load(): Boolean {
        ...
        var success = false
        try {
            if (targetFile.exists() && targetFile.length() > 0) { // 添加文件大小检查
                ...
                if (!json.isNullOrBlank()) {
                    ...
                    // 文件存在但为空，初始化默认配置
                    Log.runtime(TAG, "init $TAG config (empty file)")
                    objectMapper.updateValue(this, DataCache)
                    save()
                }
                if (oldFile.exists()) oldFile.delete()
            } else if (oldFile.exists() && oldFile.length() > 0) { // 添加文件大小检查
                if (Files.copy(oldFile, targetFile)) {
                    ...
                    if (!json.isNullOrBlank()) {
                        ...
                        // 旧文件存在但为空，初始化默认配置
                        Log.runtime(TAG, "init $TAG config (empty old file)")
                        objectMapper.updateValue(this, DataCache)
                        save()
                    }
                    oldFile.delete()
                ...
                Log.runtime(TAG, "init $TAG config (new file)")
                objectMapper.updateValue(this, DataCache)
                val formatted = JsonUtil.formatJson(this)
                if (formatted != null) {
                    save()
            ...
            try {
                ...
                Log.error(TAG, "恢复默认配置失败：${innerException.message}")
            }
        } finally {
            init = success
        }
